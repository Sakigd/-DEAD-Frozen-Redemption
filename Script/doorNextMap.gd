...
func _on_body_entered(body: Node2D):
	...
	if body.is_in_group("player") and not body.event:
		body.event = true
		player_flip_h = Game.get_singleton().player.is_flip_h()
...
static func move_to_door():
	...
	var player_camera = Game.get_singleton().PlayerCamera
	...
	Game.get_singleton().PlayerCamera.reset_smoothing()
	player.set_flip_h(player_flip_h)
